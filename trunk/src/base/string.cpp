...
int String::split(Array<String>& res, char const* sep, int flags)
{
  res.clear();
  ...
  for (int i = 0; i <= length(); i++)
  {
    if ((!strncmp(buf + i, sep, seplen) && !quote) || buf[i] == 0)
    {
      if ((flags & SPLIT_NOEMPTY) == 0 || i > prev)
      {
        String sub = substring(prev, i);
...
int String::splitList(Array<String>& res, char const* seplist, int flags)
{
...
int String::smartCompare(char const* a, char const* b)
{
  uint8_const_ptr ta = (uint8_const_ptr) a;
  uint8_const_ptr tb = (uint8_const_ptr) b;
  while (*ta || *tb)
  {
